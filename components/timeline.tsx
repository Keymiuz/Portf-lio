          ...
          {timelineEntries.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <TiltCard
                  maxTilt={6}
                  className={`ml-10 md:ml-0 ${alignRight ? 'md:col-start-3' : 'md:col-start-1'}`}
                >
                  <button
                    type="button"
                    aria-expanded={isOpen}
                    onClick={() => setOpenItemId((currentId) => (currentId === item.id ? '' : item.id))}
                    className="relative w-full rounded-2xl border border-zinc-800 bg-zinc-950/70 p-5 text-left backdrop-blur-xl transition duration-300 hover:border-red-600/80"
                  >
